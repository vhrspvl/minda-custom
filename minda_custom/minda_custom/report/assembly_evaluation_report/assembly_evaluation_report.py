...
def employee_details():
    employee = frappe.db.sql(
        """select biometric_id,employee_name,department,designation,shift,line from `tabEmployee` where status = "Active" and line like "%Ass%" """, as_dict = 1)
    return employee


def crimping_Details(emp,filters):
    crimp = frappe.db.sql(
        """select employee_code,date_of_skill_evaluatation,month_of_evaluation from `tabAssembly Evaluation` where employee_code = %s and date_of_skill_evaluatation between %s and %s """,(emp,filters.from_date,filters.to_date) , as_dict=1)
    return crimp
