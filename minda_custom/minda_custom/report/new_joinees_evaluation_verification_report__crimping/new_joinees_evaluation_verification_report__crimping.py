...
def employee_details():
    employee = frappe.db.sql(
        """select biometric_id,employee_name,shift,department,designation,line from `tabEmployee` where status = "Active" and line like "%%crimp%%" """, as_dict = 1)
    return employee


def crimping_Details(emp,filters):
    crimp = frappe.db.sql(
        """select employee_code,associate,line_name,date_of_skill_evaluatation,month_of_evaluation,date_of_joining from `tabSemi Auto Crimping` where employee_code = %s and is_evaluate=0 and date_of_skill_evaluatation between %s and %s """,(emp,filters.from_date,filters.to_date) , as_dict=1)
    return crimp
