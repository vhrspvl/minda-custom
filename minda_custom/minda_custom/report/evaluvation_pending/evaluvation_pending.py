...
def execute(filters=None):
    ...
    for e in employee:
        row = [e.biometric_id,e.employee_name,e.line]
...
def employee_details():
    employee = frappe.db.sql(
        """select biometric_id,employee_name,department,designation,line from `tabEmployee` where status = "Active" and line ="KOMAX" """, as_dict = 1)
    return employee


def crimping_Details(emp,filters):
    crimp = frappe.db.sql(
        """select employee_code,associate,line_name,date_of_skill_evaluatation,month_of_evaluation from `tabAuto Cutting and Crimping` where employee_code = %s and date_of_skill_evaluatation between %s and %s """,(emp,filters.from_date,filters.to_date) , as_dict=1)
    return crimp